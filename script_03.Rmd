...
* Defining our outcomes of interest: ugrad_sum in sep_old, and ugrad_sum in
  sep_new
* We are not considering factors that may affect enrollment other than time
* We already threw out duplicate rows


After we have a model, Temperance suggests we should:
* Acknowledge that this year is very different than other years due to COVID-19, 
  which would affect the external validity (we probably can't generalize this
  across years)
* This enrollment data only looks at students currently enrolled; many 
  undergraduates are on leave.  A fuller undergrad population may behave 
  differently.
